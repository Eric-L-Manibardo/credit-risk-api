...
from src.model.train import _catboost_frame, train_baseline
...
def test_save_creates_cbm_and_json(trained: dict, tmp_path: Path) -> None:
    ...
    assert data["model_path"] == "baseline.cbm"
...
def test_load_model_roundtrip_predicts(trained: dict, tmp_path: Path) -> None:
    ...
    proba = loaded.predict_proba(_catboost_frame(x))[:, 1]
